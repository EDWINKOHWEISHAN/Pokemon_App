// Use to manage the pokemon data
...
class PokemonProvider with ChangeNotifier {
  ...
  static const int _pageSize =
      10; // Each request retrieves basic data for 10 Pokémon.
        ...
        while (current != null) {
          ...
          if (speciesResponse.statusCode == 200) {
            ...
            if (pokemonResponse.statusCode == 200) {
              ...
              evolutions
                  .add(Pokemon.fromJson(pokemonData, evolutionOrder: order));
            }
          }
  ...
  Future<void> fetchPokemons() async {
    ...
    try {
      final response = await http.get(
        // get the basic information of pokemon on the current page from poke api
        Uri.parse(
            'https://pokeapi.co/api/v2/pokemon?limit=$_pageSize&offset=${(_currentPage - 1) * _pageSize}'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        for (var result in results) {
          ...
          if (pokemonResponse.statusCode == 200) {
            ...
            if (speciesResponse.statusCode == 200) {
              final speciesData = json.decode(speciesResponse.body);
              final evolutionChain = await _fetchEvolutionChain(
                  speciesData['evolution_chain']['url']);

              if (evolutionChain.isNotEmpty &&
                  !_isChainAlreadyAdded(evolutionChain[0].id)) {
  ...
  bool _isChainAlreadyAdded(int firstPokemonId) {
    return _evolutionChains
        .any((chain) => chain.isNotEmpty && chain[0].id == firstPokemonId);
  }
